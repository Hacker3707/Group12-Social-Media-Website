...
<div class="post-item border rounded p-3 mb-3 bg-white">
    ...
    <div class="mt-2 d-flex align-items-center">
        ...
        <?php if($isSameUser[$postId] ?? false): ?>
            <button class="btn btn-sm btn-outline-primary like-btn ml-auto"
                    type="button" data-postid="<?= $postId ?>">
    ...
    <div class="mt-2">
        <?php if($post->getPrice() !== null): ?>
            <span class="badge badge-success">💰 <?= number_format($post->getPrice()) ?> VND</span>
        <?php endif; ?>
...
document.addEventListener("click", function(e){
    ...
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4 && xhr.status === 200){
            if(xhr.responseText.trim() === "success"){ alert("Post deleted"); location.reload(); }
...
document.addEventListener("submit", function(e){
    ...
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4 && xhr.status === 200){
            if(xhr.responseText.trim() === "success"){ alert("Comment posted"); location.reload(); }
            else{ alert("Failed to post comment"); }
        }
    };
    xhr.open("POST", "index.php?controller=comment&action=addComment", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.send("postId=" + encodeURIComponent(postId) + "&content=" + encodeURIComponent(content));
});
</script>
